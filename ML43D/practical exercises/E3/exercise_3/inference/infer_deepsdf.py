...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            batch_indices = torch.randint(len(points), (self.num_samples,))
            ...
            input = torch.cat((batch_points, latent_codes), dim=1)
            predicted_sdf = model(input)

            # TODO: truncate predicted sdf between -0.1, 0.1
            predicted_sdf = torch.clamp(predicted_sdf, -self.truncation_distance, self.truncation_distance)
    ...
    def interpolate(self, shape_0_id, shape_1_id, num_interpolation_steps):
        ...
        for i in range(0, num_interpolation_steps + 1):
            ...
            weight = i/num_interpolation_steps
            interpolated_code = (1-weight)*latent_codes[0,:] + (weight)*latent_codes[1,:]
