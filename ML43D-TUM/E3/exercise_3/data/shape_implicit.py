...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        chosen_pos_points_idx = torch.randint(pos_tensor.shape[0], (int(self.num_sample_points/2),))
        chosen_neg_points_idx = torch.randint(neg_tensor.shape[0], (int(self.num_sample_points/2),))
        return torch.concat((pos_tensor[chosen_pos_points_idx], neg_tensor[chosen_neg_points_idx]))
