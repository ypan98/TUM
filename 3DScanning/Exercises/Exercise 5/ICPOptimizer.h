...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        PoseIncrement<T> pose_increasement(const_cast<T* const>(pose));
        T p_input[3], p_transformed_output[3];
        for (int i = 0; i < 3; ++i) {
            p_input[i] = T(m_sourcePoint[i]);
        }
        pose_increasement.apply(p_input, p_transformed_output);
        residuals[0] = (p_transformed_output[0] - T(m_targetPoint[0])) * T(m_weight) * T(LAMBDA);
        residuals[1] = (p_transformed_output[1] - T(m_targetPoint[1])) * T(m_weight) * T(LAMBDA);
        residuals[2] = (p_transformed_output[2] - T(m_targetPoint[2])) * T(m_weight) * T(LAMBDA);
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        pose_increasement.apply(p_input, p_transformed_output);
        residuals[0] = T(m_weight) * T(LAMBDA) * (T(m_targetNormal[0]) * (p_transformed_output[0] - T(m_targetPoint[0])) +
            T(m_targetNormal[1]) * (p_transformed_output[1] - T(m_targetPoint[1])) +
            T(m_targetNormal[2]) * (p_transformed_output[2] - T(m_targetPoint[2])));
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                double angle = std::acos(sourceNormal.dot(targetNormal) / (sourceNormal.norm() * targetNormal.norm())) * 180.0/3.1415926;
                if (angle > 60.0) {
                    match.idx = -1;
                }
            }
        ...
        for (unsigned i = 0; i < nPoints; ++i) {
            ...
            if (match.idx >= 0) {
                ...
                problem.AddResidualBlock(
                    PointToPointConstraint::create(sourcePoint, targetPoint, 1.0), nullptr, poseIncrement.getData());
                ...
                if (m_bUsePointToPlaneConstraints) {
                    ...
                    // to the Ceres problem.
                    problem.AddResidualBlock(
                        PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, 1.0), nullptr, poseIncrement.getData());
...
private:
    ...
    Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            A(4 * i, 0) = -s[2] * n[1] + s[1] * n[2];
            A(4 * i, 1) = s[2] * n[0] - s[0] * n[2];
            A(4 * i, 2) = -s[1] * n[0] + s[0] * n[1];
            A(4 * i, 3) = n[0];
            A(4 * i, 4) = n[1];
            A(4 * i, 5) = n[2];
            ...
            float weight_point_plane = 1.5f;
            A(4 * i) *= weight_point_plane;
            b(4 * i) *= weight_point_plane;
        }
