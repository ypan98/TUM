...
{
    int i_start = thread_id * height / THREADS;
    int i_end = (thread_id + 1) * height / THREADS;
    ...
    for (int i = i_start; i < i_end; i++) {
        for (int x = 0; x < width; x++) {
            ...
            for (int s = 0; s < samples; s++) {
                ...
                auto v = (float) (i + random_float()) / (height - 1);
...
int main(int argc, char **argv) {
    ...
    for (int thread_id = 0; thread_id < THREADS; ++thread_id) {
        threads[thread_id] = std::thread(kernel, thread_id);
    }
