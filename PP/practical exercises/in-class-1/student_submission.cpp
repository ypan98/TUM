 ...
 */
void decrypt_message(uint8_t *decryptedMessage, uint8_t *keys, uint8_t *values, uint8_t* dict)
{
    for (unsigned int i = 0; i < STRING_LEN; ++i)
    {
        uint8_t key = decryptedMessage[i];
        decryptedMessage[i] = dict[key];
    }
}
...
int main()
{
    ...
    */
    ...
    // use dict to decrypt message
    for (int i = 0; i < NUM_ITERATIONS; i++)
    {
        decrypt_message(decryptedMessage, keys, values, dict);
    }
